"""
Module: GeneratedDataLoader

This module provides functionality to generate and load synthetic data into a database. 
It uses the `DataGenerator` class to create mock data for facilities, patients, and visits, 
and ensures that the data is injected into the appropriate database tables.

Classes:
    - GeneratedDataLoader: Handles the creation of tables, checks for existing data, 
      and injects generated data into the database.

Dependencies:
    - DataGenerator: A class responsible for generating synthetic data.
    - SQL Queries: Predefined SQL queries for creating tables and inserting data.

Usage:
    1. Initialize the `GeneratedDataLoader` with a database connection.
    2. Call the `inject_data` method to create tables (if they don't exist) and load data 
       into the database if the tables are empty.
